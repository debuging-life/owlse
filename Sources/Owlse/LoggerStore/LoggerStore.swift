...
public final class LoggerStore: @unchecked Sendable, Identifiable {
    ...
    private let queue = DispatchQueue(label: "com.github.kean.owlse.logger-store")
    ...
    private static func makeDefault() -> LoggerStore {
        let storeURL = URL.logs.appending(directory: "current.owlse")
    ...
    /// you are ready to share the store, create a Owlse document using ``export(to:options:)``
    ...
    public init(storeURL: URL, options: Options = [.create, .sweep], configuration: Configuration = .init()) throws {
        ...
        if var manifest = Manifest(url: manifestURL) {
            if manifest.version != .currentStoreVersion {
                // Upgrading to a new version of Owlse store
            ...
            if Files.fileExists(atPath: databaseURL.path) {
                // Updating from Owlse 1.0 that didn't have a manifest file
                try? LoggerStore.removePreviousStore(at: storeURL)
            }
...
extension LoggerStore {
    ...
    /// Toggles the pin on a task, writing through the store's own background
    /// context. Never mutate entities on `viewContext`: the background
    /// context caches its snapshots and has no merge policy, so a conflicting
    /// save there fails permanently and silently stops all further capture.
    public func togglePin(for objectID: NSManagedObjectID) {
        perform { context in
            guard let task = try? context.existingObject(with: objectID) as? NetworkTaskEntity else { return }
...
extension LoggerStore {
    ...
    private func _exportAsArchive(to targetURL: URL, options: ExportOptions) async throws {
        if options.predicate != nil || options.sessions != nil {
            ...
            let tempStoreURL = temporary.url.appending(filename: "temp.owlse")
    ...
    private func _exportPackageAsArchive(to targetURL: URL) async throws {
        ...
        let document = try OwlseDocument(documentURL: targetURL)
        var totalSize: Int64 = 0

        return try document.context.performAndReturn {
            // Add database
            let documentBlob = OwlseBlobEntity(context: document.context)
            ...
            if Files.fileExists(atPath: blobsURL.path) {
                ...
                for chunk in blobURLs.chunked(into: 100) {
                    ...
                    try document.context.execute(NSBatchInsertRequest(entityName: String(describing: OwlseBlobEntity.self), objects: objects))
                }
            }
            ...
            let infoBlob = OwlseBlobEntity(context: document.context)
            infoBlob.key = "info"
...
extension LoggerStore {
    public enum Error: Swift.Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case let .unsupportedVersion(version, minimumSupportedVersion): return "The store was created by with Owlse vesrion \(version). Minimum supported version is \(minimumSupportedVersion)."
