...
extension RemoteLogger {
    package final class Connection: @unchecked Sendable  {
        ...
        /// Touched from both the main queue (sendMessage) and the connection
        /// queue (responses), so every access takes this lock.
        ...
        private let queue = DispatchQueue(label: "com.github.kean.owlse.remote-logger-connection")
        ...
        package init(_ connection: NWConnection, delegate: RemoteLoggerConnectionDelegate? = nil) {
            ...
            self.log = isLogEnabled ? OSLog(subsystem: "com.github.kean.owlse", category: "RemoteLogger") : .disabled
        }
        ...
        private func send(event: Event) {
            ...
            if case .packet(let packet) = event,
               ...
               let handler = { handlersLock.lock(); defer { handlersLock.unlock() }
                               return handlers.removeValue(forKey: header.id) }() {
        ...
        package func sendMessage(path: Path, data: Data? = nil, _ completion: ((Data?, Error?) -> Void)? = nil) {
            ...
            if let completion = completion {
                ...
                connection.queue?.asyncAfter(deadline: .now() + .seconds(20)) { [weak self] in
                    ...
                    let handler = self.handlers.removeValue(forKey: id)
...
extension NWParameters {
    /// The label used to derive the TLS pre-shared key from the passcode.
    /// Clients built with the Owlse SDK use ``owlse``; apps built with the
    /// original Pulse SDK derive theirs from ``legacyPulse`` — the server's
    /// legacy listener must match it or protected connections cannot
    /// complete the TLS handshake.
    package enum ProtocolLabel: String {
        case owlse = "OwlseLoggerProtocol"
        case legacyPulse = "PulseLoggerProtocol"
    }

    package convenience init(passcode: String, protocolLabel: ProtocolLabel = .owlse) {
        let tcpOptions = NWProtocolTCP.Options()
        tcpOptions.enableKeepalive = true
        tcpOptions.keepaliveIdle = 4

        self.init(tls: NWParameters.tlsOptions(passcode: passcode, label: protocolLabel.rawValue), tcp: tcpOptions)

        self.includePeerToPeer = true
    }

    // Create TLS options using a passcode to derive a preshared key.
    private static func tlsOptions(passcode: String, label: String) -> NWProtocolTLS.Options {
        let tlsOptions = NWProtocolTLS.Options()

        let authenticationKey = SymmetricKey(data: passcode.data(using: .utf8)!)
        var authenticationCode = HMAC<SHA256>.authenticationCode(for: label.data(using: .utf8)!, using: authenticationKey)
        ...
        sec_protocol_options_add_pre_shared_key(
            ...
            DispatchData.make(string: label) as __DispatchData
        )
