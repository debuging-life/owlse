...
private let rootURL = FileManager.default.temporaryDirectory.appendingPathComponent("owlseui-demo")
...
private func makeMockStore(configuration: LoggerStore.Configuration = .init()) -> LoggerStore {
    ...
    let storeURL = rootURL.appendingPathComponent("\(UUID().uuidString).owlse")
...
private func _syncPopulateStore(_ store: LoggerStore) {
    ...
    _logTask(.uploadOwlseArchive, urlSession: urlSession, logger: networkLogger)
