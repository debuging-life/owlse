...
extension MockTask {
    ...
    package static var allTasks: [MockTask] = [.login, .profile, .repos, .octocat, .downloadNuke, .createAPI, .uploadOwlseArchive, .patchRepo]
    ...
    package static let uploadOwlseArchive = MockTask(
        kind: .upload(size: 21851748),
        originalRequest: mockUploadOwlseOriginalRequest,
        response: mockUploadOwlseResponse,
        responseBody: Data(),
        transactions: [
            .init(fetchType: .networkLoad, request: mockUploadOwlseCurrentRequest, response: mockUploadOwlseResponse, duration: 2.21283, isReusedConnection: true)
        ],
        delay: 6.5,
        taskDescription: "upload-owlse-archive"
    )
    ...
    package static let notifications = _mockTask(
        ...
        responseBody: #"[{"id":"1","reason":"subscribed","subject":{"title":"Fix memory leak in ImagePipeline","url":"https://api.github.com/repos/kean/nuke/issues/432","type":"Issue"},"updated_at":"2024-01-15T10:30:00Z"},{"id":"2","reason":"mention","subject":{"title":"Add Swift 6 support","url":"https://api.github.com/repos/kean/owlse/pulls/89","type":"PullRequest"},"updated_at":"2024-01-15T09:15:00Z"}]"#,
        duration: 0.22
    )
    ...
    package static let userEvents = _mockTask(
        ...
        responseBody: #"[{"type":"PushEvent","repo":{"name":"kean/nuke"},"created_at":"2024-01-15T11:00:00Z","payload":{"commits":[{"message":"Fix image cache eviction"}]}},{"type":"IssuesEvent","repo":{"name":"kean/owlse"},"created_at":"2024-01-15T10:30:00Z","payload":{"action":"opened","issue":{"title":"Console crash on watchOS"}}}]"#,
        duration: 0.26
    )
    ...
    package static let graphQL = _mockTask(
        ...
        responseBody: #"{"data":{"viewer":{"login":"kean","repositories":{"nodes":[{"name":"Nuke","stargazerCount":8142},{"name":"Owlse","stargazerCount":6280},{"name":"Align","stargazerCount":845},{"name":"Get","stargazerCount":812},{"name":"CreateAPI","stargazerCount":465}]}}}}"#,
        duration: 0.55,
    ...
    package static let updateProfile = _mockTask(
        ...
        method: "PATCH",
        requestBody: #"{"bio":"Creator of Nuke, Owlse, and other open-source Swift libraries","location":"New York","hireable":false}"#,
        responseBody: #"{"login":"kean","id":1567433,"bio":"Creator of Nuke, Owlse, and other open-source Swift libraries","location":"New York","hireable":false,"public_repos":42,"followers":2891}"#,
        duration: 0.29
    )
...
private let mockUploadOwlseOriginalRequest = URLRequest(url: "https://objects-origin.githubusercontent.com/github-production-release-asset-2e65be/upload-we9zs7v.zip", method: "POST", headers: [
    "Content-Length": "21851748",
    "Content-Type": "multipart/form-data; boundary=----WebKitFormBoundaryrv8XAHQPtQcWta3k"
])

private let mockUploadOwlseCurrentRequest = mockUploadOwlseOriginalRequest.adding(headers: [
    "User-Agent": "Owlse Demo/2.0",
    "Accept-Encoding": "gzip",
    "Accept-Language": "en-us",
    "Accept": "*/*"
...
private let mockUploadOwlseResponse = HTTPURLResponse(url: "https://objects-origin.githubusercontent.com/github-production-release-asset-2e65be/upload-we9zs7v.zip", statusCode: 204, headers: [
    "Vary": "Origin",
